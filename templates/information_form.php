...
<table class="table table-condensed table-striped table-bordered" id="assets" style="border-collapse:collapse;">
    ...
    echo('<td >Dividend: ' . number_format($asset["dividend"], 2, ".", ",") .
        ...
        '</td>');
    echo('</tr>');
    echo('<tr><td colspan="4">Portfolio: ' . number_format($asset["quantity"]), 0, '.', ',') . '%</td></tr>');
    echo('<tr><td colspan="4">Orderbook: ' . number_format($asset["locked"]), 0, '.', ',') . '%</td></tr>');
    echo('<tr><td colspan="4">Total: ' . number_format($asset["locked"]+$asset["quantity"]), 0, '.', ',') . '%</td></tr>');
    echo('<tr><td colspan="4">Control: ' . number_format(($asset["control"]), 0, '.', '') . '%</td></tr>');
