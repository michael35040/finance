...
if(!empty($_GET["fontcolor"])){
    $fontcolor = ltrim($_GET["fontcolor"], '#');
    if(!ctype_alnum($fontcolor)) {exit;}
    }else{$fontcolor = "000000";}
if(!empty($_GET["backgroundcolor"])){$backgroundcolor = ltrim($_GET["backgroundcolor"], '#');
    if(!ctype_alnum($backgroundcolor))  {exit;}
    }else{$backgroundcolor = '808080';}
if(!empty($_GET["shadow"])){$shadow = ltrim($_GET["shadow"], '#');
    if(!ctype_alnum($shadow))           {exit;}
    }else{$shadow = "202020";}
if(!empty($_GET["height"])){$height = $_GET["height"];
    if(!ctype_alnum($height))           {exit;}
    }else{$height = '100';}
if(!empty($_GET["width"])){$width = $_GET["width"];
    if(!ctype_alnum($width))            {exit;}
    }else{$width = '200';}
if(!empty($_GET["text"])){$text = $_GET["text"];
    if(!ctype_alnum($text))             {exit;}
    }else{$text = '';}
if(!empty($_GET["size"])){$size = $_GET["size"];
    if(!ctype_alnum($size))             {exit;}
    }else{$size = '20';}
if(!empty($_GET["rotation"])){$rotation = $_GET["rotation"];
    if(!ctype_alnum($rotation))         {exit;}
    }else{$rotation = '0';}
if(!empty($_GET["price"])){$price = $_GET["price"];
   // if(!ctype_alnum($price))            {exit;}
    }else{$price = '';}
if(!empty($_GET["name"])){$name = $_GET["name"];
   //if(!ctype_alnum($name))             {exit;} //HAS ()
    }else{$name = '';}
if(!empty($_GET["quantity"])){$quantity = $_GET["quantity"];
    //if(!ctype_alnum($quantity))         {exit;}
}else{$quantity = '';}
